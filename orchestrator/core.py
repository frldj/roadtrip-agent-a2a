...
async def _timed_agent_call(agent_name: str, url: str, payload: dict) -> dict:
    """Appelle un agent A2A en enregistrant durée et statut dans Prometheus."""
    t = time.perf_counter()
    status = "success"
    try:
...
async def plan_roadtrip(
    ...
    try:
        ...
        if "error" in charging_raw:
            charging = None
        else:
            charging = ChargingPlanResponse.model_validate(charging_raw)
